...
import 'package:zoncan/features/accounts/domain/repository/user_details_repository.dart';
...
class AuthenticationRepositoryImpl extends AuthenticationRepository {
  ...
  currentUserDetails() async {
    try {
      ...
      if (user != null) {
        ...
        ); // Convert JSON to UserDetailsModel and return as success
      } else {
  ...
  Future<Either<FailureException, bool>> emailAddressAlreadyExists(
    ...
    return await _userDetailsRepository.userExist(
      email,
    ); // True if email exists, failure if error occurs
  }
  ...
  Future<Either<FailureException, UserDetailsModel>> login(
    ...
    return cacheResult.fold(
      ...
      (inCacheUser) async {
        ...
        if (!inCacheUser.isEmpty && username == inCacheUser.userName) {
          final verifyResult = await _verifyPassword(
            inCacheUser.encryptedPassword!,
            password,
          ); // Verify cached user's password
          return verifyResult.fold(
            ...
            (isValid) =>
                isValid
                    ? Right(
                      inCacheUser,
                    ) // Return cached user if password matches
                    : Left(
                      FailureException(
                        ...
                        userMessage:
                            TranslationsProvider
                                .translator
                                .accounts
                                .incorrectPassword, // User-friendly error
                      ),
                    ),
          );
        } else {
                      ...
                      FailureException(
                        ...
                        userMessage:
                            TranslationsProvider
                                .translator
                                .accounts
                                .userNotSaved, // User-friendly error
                      ),
                    );
                  }
                    ...
                    try {
                      ...
                      return Left(
                        FailureException(
                          ...
                          error: e,
                          stackTrace: stackTrace,
                          message: "Failed to delete remember me data: $e",
                        ),
  ...
  Future<Either<FailureException, bool>> logout() async {
    try {
      return await _storage
          .delete(SecurityKeys.userDetails)
          .then(
            (res) =>
                res
                    ? Right(res) // Success if deletion worked
                    : Left(FailureException(userMessage: "User logout failed")),
          ); // Failure if deletion didn't work
    } catch (e, stackTrace) {
      return Left(
        FailureException(
          type: ExceptionType.FAILED,
      ...
      return await _storageProvider.read(_rememberMeUsrKey).then((res) {
        if (res != null) {
          ...
          return Left(
            FailureException(
              userMessage:
                  TranslationsProvider.translator.accounts.notValidUsername,
            ),
          ); // Failure if not found
        }
      });
    ...
    if (user.email == null) {
      return Left(
        FailureException(
          ...
          userMessage: TranslationsProvider.translator.validation.notValidEmail,
        ),
      );
    }
      ...
      (emailExists) async {
        ...
        return usernameCheck.fold(
          ...
          (usernameExists) async {
            ...
            try {
              final hash = await _passwordEncryption.hashB64(
                password,
              ); // Hash the password
              final userDetails = user.copyWith(
                encryptedPassword: hash,
              ); // Update user with hashed password
              final savedUser = await _userDetailsRepository.saveUser(
                userDetails,
              ...
              return savedUser.fold(
                ...
                (userModel) async {
                  final isUserInCache = await _saveUserDetails(
                    userModel,
                  ); // Save to cache
                  if (isUserInCache) {
  ...
  Future<Either<FailureException, bool>> usernameAlreadyExists(
    ...
    return await _userDetailsRepository.userExist(
      username,
    ); // True if username exists, failure if error
  }
  ...
  Future<bool> _saveUserDetails(UserDetailsModel user) async => _storage.write(
    SecurityKeys.userDetails,
    user.toJson(),
  ); // Write user JSON to storage and return success status

  // Verify a password against its hashed version
  Future<Either<FailureException, bool>> _verifyPassword(
    String b64HashedPassword,
    String password,
  ) async {
    try {
      return Right(
        await _passwordEncryption.verifyB64(password, b64HashedPassword),
